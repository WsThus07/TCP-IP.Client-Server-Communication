...
#define PORTCLIENT 8092



int Port[3]={8092,8093};
...
////@Server_side
	...
	if ((bind(sockfd, (struct sockaddr*)&servaddr, sizeof(servaddr))) != 0) {
        ...
        printf("Router 2 Binding successfully...\n");
	if ((listen(sockfd, 5)) != 0) {
		printf("router 2 Listening failed...\n");
		exit(0);
	}
	printf("Router 2 is Listening successfully...\n");
	len = sizeof(cli);
	connfd = accept(sockfd, (struct sockaddr*)&cli, &len);
	if (connfd < 0) {
		printf("Router 2 Failed accepting...\n");
		exit(0);
	}
	printf("Router 2 is Listening to Client...\n");
...
////@Client_side
	...
	if (connect(sockfd2, (struct sockaddr*)&servaddr2, sizeof(servaddr2)) != 0) {
		printf("connection with the router 2 failed...\n");
		exit(0);
	}
	else
		printf("connected to the router 2 ..\n");
